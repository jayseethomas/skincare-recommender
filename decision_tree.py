...
class DecisionTree(object):
	...
	def getEntropy(self, sampleIds):
		...
		for lv in labelCount:
			...
			if lv != 0:
				...
				# print((-lv/len(sampleIds) * math.log(lv/len(sampleIds), 2)))
			else:
				entropy += 0
				# print(0)
		return entropy
	...
	def getInformationGain(self, sampleIds, attributeId):
		...
		for vc, vids in zip(attributeValsCount, attributeValsIds):
			...
			# print(vc/len(sampleIds) * self.getEntropy(vids))
		return gain
...
def test():
	f = open('/Users/JJThomas/PycharmProjects/decision_tree/venv/playtennis')
